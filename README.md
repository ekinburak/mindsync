...
|---------|-------------|
| `/llm-wiki init` | One-time setup: creates vault structure, personalized CLAUDE.md, configures tools |
| `/llm-wiki ingest` | Interactive ingest of a new source (URL, file, paste, video) |
| `/llm-wiki lint` | Health-check: finds orphans, contradictions, gaps, stale claims |
...
```bash
curl -s https://raw.githubusercontent.com/YOUR_USERNAME/llm-wiki/main/install.sh | bash
```
...
## Recommended tools (installed automatically by `/llm-wiki init`)
...
4. Run `/llm-wiki init` and follow the prompts
5. Drop a source into `raw/` and run `/llm-wiki ingest`
...
Each vault is self-contained. Run `/llm-wiki init` in a different folder to create a new wiki (e.g. a research wiki, a project wiki). Each gets its own `CLAUDE.md` tuned to its domain.

## Auto-ingest with file watcher

During `/llm-wiki init`, you can optionally set up a file watcher. Once running, any file dropped into `raw/` automatically triggers the ingest flow. Requires `fswatch` (`brew install fswatch`).
